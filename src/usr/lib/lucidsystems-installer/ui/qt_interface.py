#!/usr/bin/env python
import sys
sys.path.append('/usr/lib/lucidsystems-installer')
sys.path.append('/usr/share/lucidsystems-installer')
import os
import commands
import GeoIP
import gettext
...
class InstallerWindow(QtGui.QMainWindow):
    ...
    resource_dir = '/usr/share/lucidsystems-installer/'

    def __init__(self):
        QtGui.QMainWindow.__init__(self)
 
        self.ui = uic.loadUi('/usr/share/lucidsystems-installer/qt_interface.ui')
        self.ui.show()
    ...
    def setCurrentPageIndex(self, index):
        ...
        if index != self.getCurrentPageIndex():
            ...
            if (index <= self.PAGE_WELCOME):
                ...
                self.ui.headLabel.setText(unicode("Welcome to the %s installation" % self.DISTRIBUTION_NAME))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/lucidsystems_icon_blue_32x32.png"))
            elif (index is self.PAGE_LANGUAGE):
                self.ui.headLabel.setText(unicode("Choose your language"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/locales.png"))
                self.build_language_list()
            elif (index is self.PAGE_TIMEZONE):
                self.ui.headLabel.setText(unicode("Choose your timezone"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/time.png"))
                self.build_timezone_list()
            elif (index is self.PAGE_KEYBOARD):
                self.ui.headLabel.setText(unicode("Choose your keyboard layout"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/keyboard.png"))
                self.build_keyboard_lists()
            elif (index is self.PAGE_HARDDISK):
                self.ui.headLabel.setText(unicode("Select Harddisk"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/hdd.svg"))
                ...
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/advanced.png"))
                self.build_bootloader_list()                
            elif (index is self.PAGE_USER):
                self.ui.headLabel.setText(unicode("Create your user account"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/user.png"))
                ...
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/summary.png"))
                ...
                if (self.setup.internet_connectivity == True):
                    self.ui.connectivityIcon.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/actions/dialog-ok-3.png"))
                else:
                    self.ui.connectivityIcon.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/actions/dialog-no-2.png"))
            elif (index is self.PAGE_INSTALL):
                self.ui.headLabel.setText(unicode("Installation"))
                self.ui.headLogo.setPixmap(QtGui.QPixmap("/usr/share/lucidsystems-installer/icons/install.png"))
    ...
    def build_partitions(self):           
        os.popen('mkdir -p /tmp/lucidsystems-installer/tmpmount')
        ...
        try:                                                                                            
            ...
            if self.setup.target_disk is not None:
                ...
                try:
                    ...
                    dialog = QuestionDialog("lucidsystems Installer", "No partition table was found on the hard drive. Do you want the installer to create a set of partitions for you? Note: This will erase any data present on the disk.")
                    if (dialog.show()):
                ...
                while (partition is not None):
                    if last_added_partition.partition.number == -1 and partition.number == -1:
                        ...
                        if partition.number != -1 and "swap" not in last_added_partition.type and partition.type != parted.PARTITION_EXTENDED:
                            # Umount temp folder
                            if ('/tmp/lucidsystems-installer/tmpmount' in commands.getoutput('mount')):
                                os.popen('umount /tmp/lucidsystems-installer/tmpmount')

                            # Mount partition if not mounted
                            if (partition.path not in commands.getoutput('mount')):                                
                                os.system("mount %s /tmp/lucidsystems-installer/tmpmount" % partition.path)
                            ...
                            if (partition.path in commands.getoutput('mount')):
                                ...
                                for df_line in df_lines:
                                    ...
                                    if df_elements[0] == partition.path:
                                        ...
                                        if os.path.exists(os.path.join(mount_point, 'etc/lucidsystems-version')):
                                            last_added_partition.description = "lucidsystems"
    ...
    def check_connectivity(self):
        ...
        try:
            con = urllib2.urlopen("http://www.lucidsystems.org")
            data = con.read()
            return True
        except:
            try:
...
class PartitionEditDialog(object):
    def __init__(self, base, partition_path, format_as, mount_point):       
        self.partitionEditBox = uic.loadUi('/usr/share/lucidsystems-installer/qt_partition_edit_dialog.ui')
